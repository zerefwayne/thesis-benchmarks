#!/bin/bash
#SBATCH --job-name=osu_bibw_fixed_rndv
...
#
# osu_bibw_fixed_rndv.sh — bidirectional pt2pt bw (12 pairs) with UCX
# rendezvous tuning.
#
# Companion to osu_bibw.sh. Only difference: EESSI arm exports
# UCX_RNDV_THRESH=1024 to bypass the 256B protocol-switch cliff observed
# in osu_bibw_eessi (ratio e/n ≈ 0.12 at 256B across all 12 pairs).
#
# Same threshold rationale as osu_bw_fixed_rndv.sh — see fix_rndv_reasoning.md.
# Note: bibw also shows a ~20% small-msg (1B-128B) gap that is NOT a threshold
# problem — likely a UCX bidirectional-eager protocol overhead.
#
# Submit:
...
FILE_BASE="osu_bibw_fixed_rndv_${STACK}"
...
run_pair_native() {
    ...
    { echo; echo "# [native sdma=$sdma] $label  GCD($g0,$g1) tier=$tier links=$nlinks"; } | tee -a "$LOG_FILE"
...
run_pair_eessi() {
    ...
    local topo tier nlinks
    topo=$(get_topology "$g0" "$g1"); tier="${topo%,*}"; nlinks="${topo#*,}"
    { echo; echo "# [eessi sdma=$sdma rndv=$UCX_RNDV_THRESH] $label  GCD($g0,$g1) tier=$tier links=$nlinks"; } | tee -a "$LOG_FILE"
    for run in $(seq 1 $NUM_RUNS); do
        local t0=$SECONDS
        if (( run == WARMUP_RUN )); then
            echo "  [run $run/$NUM_RUNS] warm-up — $(date '+%H:%M:%S')" | tee -a "$LOG_FILE"
            timeout $RUN_TIMEOUT bash -c \
                "ROCR_VISIBLE_DEVICES=${g0},${g1} mpirun -n 2 -x UCX_RNDV_THRESH $bin $OSU_FLAGS" \
                > /dev/null 2>&1
            echo "  [run $run/$NUM_RUNS] warm-up done in $((SECONDS - t0))s" | tee -a "$LOG_FILE"
            continue
        fi
...
if [[ "$STACK" == "native" ]]; then
    ...
    for entry in "${A1_PAIRS[@]}"; do
        ...
        run_pair_native 0 "$g0" "$g1" "$label" "${OSU_NATIVE_PT2PT}/osu_bibw"
    done
else
    ...
    for entry in "${A1_PAIRS[@]}"; do
        read -r g0 g1 label <<< "$entry"
        run_pair_eessi 0 "$g0" "$g1" "$label" "${OSU_PT2PT}/osu_bibw"
    done
fi
