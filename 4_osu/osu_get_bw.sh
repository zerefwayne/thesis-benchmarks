...
#
# osu_get_bw.sh — A8b: one-sided RMA get bandwidth (12 pairs)
# MPI_Get path. One stack per job. Submit:
#   sbatch osu_get_bw.sh eessi
...
if [[ "$STACK" == "native" ]]; then
    ...
    for entry in "${A1_PAIRS[@]}"; do
        ...
        run_pair_native 0 "$g0" "$g1" "$label" "${OSU_NATIVE_ONESIDED}/osu_get_bw"
    done
else
    ...
    for entry in "${A1_PAIRS[@]}"; do
        read -r g0 g1 label <<< "$entry"
        run_pair_eessi 0 "$g0" "$g1" "$label" "${OSU_ONESIDED}/osu_get_bw"
    done
fi
