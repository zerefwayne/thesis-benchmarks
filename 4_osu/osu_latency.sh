...
#
# osu_latency.sh — A1 / A2 ping-pong latency (12 pairs)
...
echo "stack,sdma_enabled,pair_label,gcd_a,gcd_b,tier,num_links,run,size_bytes,latency_us" \
    > "$CSV_FILE"

NUM_RUNS=6
WARMUP_RUN=1
RUN_TIMEOUT=300
# OSU latency default max is 1 MiB. Past that, "latency" is just
# transfer-size / bandwidth — covered by osu_bw. Keep the default.
OSU_FLAGS="-i 100 -d rocm D D"
...
if [[ "$STACK" == "native" ]]; then
    ...
    for entry in "${A1_PAIRS[@]}"; do
        ...
        run_pair_native 0 "$g0" "$g1" "$label" "${OSU_NATIVE_PT2PT}/osu_latency"
    done
else
    ...
    for entry in "${A1_PAIRS[@]}"; do
        read -r g0 g1 label <<< "$entry"
        run_pair_eessi 0 "$g0" "$g1" "$label" "${OSU_PT2PT}/osu_latency"
    done
fi
