...
#
# osu_collectives.sh — A4 intra-node collectives at N ∈ {2, 4, 8}
# Binaries: osu_allreduce, osu_alltoall, osu_bcast, osu_allgather
# GCD subsets (matching 3_osu_eessi_thesis/osu_allreduce.sh:GCD_FOR_N):
#   N=2 → 0,1        (intra-package, 4-link)
#   N=4 → 0,1,2,3    (2 full packages)
#   N=8 → 0..7       (full node)
# N=6 omitted on purpose — standard-g nodes are 4 packages × 2 GCDs and
# real workloads rarely use 6 GCDs in this geometry.
#
...
# Native uses a wrapper script for ROCR_VISIBLE_DEVICES — same idiom for
# any N, matches 3_osu_native_thesis/osu_alltoall.sh:22-30.
...
run_eessi() {
    local sdma=$1 benchmark=$2 bin=$3 ng=$4 devices=$5
    { echo; echo "# [eessi sdma=$sdma] $benchmark  N=$ng  devices=$devices"; } | tee -a "$LOG_FILE"
    for run in $(seq 1 $NUM_RUNS); do
        local t0=$SECONDS
        if (( run == WARMUP_RUN )); then
            echo "  [run $run/$NUM_RUNS] warm-up — $(date '+%H:%M:%S')" | tee -a "$LOG_FILE"
            timeout $RUN_TIMEOUT bash -c \
                "ROCR_VISIBLE_DEVICES=$devices mpirun -n $ng $bin -d rocm" \
                > /dev/null 2>&1
            echo "  [run $run/$NUM_RUNS] warm-up done in $((SECONDS - t0))s" | tee -a "$LOG_FILE"
            continue
        fi
...
if [[ "$STACK" == "native" ]]; then
    ...
    # UCC tuning — only for EESSI collectives. Routes through UCP+self,
    # drops RCCL TL to avoid ~26us launch overhead at small sizes.
    # Rationale: 3_osu_eessi_thesis/osu_allreduce.sh:17-36.
