#!/bin/bash
#SBATCH --job-name=osu_bw_fixed_rndv
...
#
# osu_bw_fixed_rndv.sh — pt2pt bw (12 pairs) with UCX rendezvous tuning.
#
# Companion to osu_bw.sh. Only difference: EESSI arm exports
# UCX_RNDV_THRESH=1024 to bypass the 256B protocol-switch cliff observed
# in osu_bw_eessi (ratio e/n = 0.06 at 256B with DEFAULT threshold).
#
# Justification for 1024 (from osu_protocol_eessi_18634230 sweep):
#   - DEFAULT (~256B): cliff at 256B-4K, depth 5-15x vs native
#   - 1024:            cliff at 512B-4K, depth 5-20x; 256B fixed (+25x)
#   - 16777216 (16M):  bulk capped at ~660 MB/s up to 8 MiB — catastrophic
# 1024 strictly dominates DEFAULT; higher values regress bulk bandwidth.
# Residual 512B-4K weakness is intrinsic to UCX on MI250X, not a knob.
#
# Native arm runs untouched — osu_protocol_native showed no cliff in DEFAULT.
#
# Submit:
#   sbatch osu_bw_fixed_rndv.sh eessi
...
FILE_BASE="osu_bw_fixed_rndv_${STACK}"
...
# Schema mirrors osu_bw.sh exactly so CSVs can be concat'd / compared directly.
...
run_pair_eessi() {
...

    {
        echo
        echo "################################################################"
        echo "# [eessi sdma=$sdma rndv=$UCX_RNDV_THRESH] $label  GCD($g0,$g1) tier=$tier links=$nlinks"
        echo "################################################################"
    } | tee -a "$LOG_FILE"
...
if [[ "$STACK" == "native" ]]; then
    ...
    echo "######## EESSI STACK (HSA_ENABLE_SDMA=0, UCX_RNDV_THRESH=1024) ########" | tee -a "$LOG_FILE"
