#!/bin/bash
#SBATCH --job-name=osu_mbw_mr_fixed_rndv
...
#
# osu_mbw_mr_fixed_rndv.sh — multi-pair bandwidth with UCX rndv tuning.
#
# Companion to osu_mbw_mr.sh. Only difference: EESSI arm exports
# UCX_RNDV_THRESH=1024 to attack the multi-pair cliff observed in
# osu_mbw_mr_eessi_18634824 vs native_18634827:
#   128 B → e/n = 0.04 (25× slower; msg rate 0.52M vs 13M Mmsgs/s)
#   256 B → e/n = 0.08
#   1-4K  → e/n = 0.23-0.24
# Cliff kicks in one size earlier than osu_bw (128B vs 256B) because mbw_mr
# uses 64 outstanding messages per pair, so the protocol switch is sensitive
# to the underlying eager/rndv decision sooner.
#
# Same threshold rationale as osu_bw_fixed_rndv.sh — see fix_rndv_reasoning.md.
#
# Submit:
#   sbatch osu_mbw_mr_fixed_rndv.sh eessi
#   sbatch osu_mbw_mr_fixed_rndv.sh native     # baseline reference
...
FILE_BASE="osu_mbw_mr_fixed_rndv_${STACK}"
...
# Three ROCR_VISIBLE_DEVICES orderings — see osu_mbw_mr.sh.
...
run_eessi() {
    local sdma=$1 cfg_lab=$2 pd=$3 rocr=$4 bin=$5
    { echo; echo "# [eessi sdma=$sdma rndv=$UCX_RNDV_THRESH] $cfg_lab  ROCR=$rocr"; } | tee -a "$LOG_FILE"
    for run in $(seq 1 $NUM_RUNS); do
        local t0=$SECONDS
        if (( run == WARMUP_RUN )); then
            echo "  [run $run/$NUM_RUNS] warm-up — $(date '+%H:%M:%S')" | tee -a "$LOG_FILE"
            timeout $RUN_TIMEOUT bash -c \
                "ROCR_VISIBLE_DEVICES=$rocr mpirun -n 8 -x UCX_RNDV_THRESH $bin $OSU_FLAGS" \
                > /dev/null 2>&1
            echo "  [run $run/$NUM_RUNS] warm-up done in $((SECONDS - t0))s" | tee -a "$LOG_FILE"
            continue
        fi
