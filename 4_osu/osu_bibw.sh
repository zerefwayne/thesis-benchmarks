...
#
# osu_bibw.sh — A1 / A2 bidirectional bandwidth (12 pairs)
...
WARMUP_RUN=1
RUN_TIMEOUT=300
# 256 MiB cap — bw saturates by ~64 MiB on every tier per 3_osu_eessi_thesis
# data (e.g. (0,1) at 1 GiB = 142 GB/s vs 64 MiB = 139 GB/s, only +2%).
# Going past 256 MiB just spends compute confirming the plateau.
...
run_pair_eessi() {
    ...
    local topo tier nlinks
    topo=$(get_topology "$g0" "$g1"); tier="${topo%,*}"; nlinks="${topo#*,}"
    { echo; echo "# [eessi sdma=$sdma] $label  GCD($g0,$g1) tier=$tier links=$nlinks"; } | tee -a "$LOG_FILE"
    for run in $(seq 1 $NUM_RUNS); do
        local t0=$SECONDS
        if (( run == WARMUP_RUN )); then
            echo "  [run $run/$NUM_RUNS] warm-up — $(date '+%H:%M:%S')" | tee -a "$LOG_FILE"
