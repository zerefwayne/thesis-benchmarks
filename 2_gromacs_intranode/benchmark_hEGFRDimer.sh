#!/bin/bash
#SBATCH --job-name=gromacs_hEGFRDimer
...
#
# hEGFRDimer (~465k atoms). Mid-size: PME-on-GPU sweet spot.
# Mirrors 2_gbs_eessi/benchmark_hEGFRDimer.sh — no -update gpu.
# Per-run wall ~10-15 min; 7 runs ~1.5-2 h.
#
# Submit:
...
WARMUP_RUN=1
INPUT_TPR="$(pwd)/GROMACS_Benchmark_Suite/HECBioSim/hEGFRDimer/benchmark.tpr"
