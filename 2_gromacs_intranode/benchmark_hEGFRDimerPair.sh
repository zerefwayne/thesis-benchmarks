#!/bin/bash
#SBATCH --job-name=gromacs_hEGFRDimerPair
...
#
# hEGFRDimerPair (~3M atoms). Large: memory + xGMI bandwidth bound.
# Per-run wall ~15-25 min; 7 runs ~2-3 h. dev-g max time is 3 h.
#
# Submit:
...
WARMUP_RUN=1
INPUT_TPR="$(pwd)/GROMACS_Benchmark_Suite/HECBioSim/hEGFRDimerPair/benchmark.tpr"
