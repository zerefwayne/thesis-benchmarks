...
RESULT_DIR="results"
mkdir -p "$RESULT_DIR"
CSV_FILE="$RESULT_DIR/osu_latency_${SLURM_JOB_ID}.csv"
LOG_FILE="$RESULT_DIR/osu_latency_${SLURM_JOB_ID}.log"
...
run_pair() {
    ...
    for run in $(seq 1 $NUM_RUNS); do
        if (( run == WARMUP_RUN )); then
            ...
            ROCR_VISIBLE_DEVICES=$g0,$g1 mpirun -n 2 ${OSU_PT2PT}/osu_latency -d rocm D D \
                > /dev/null 2>&1
            continue
        fi
        ...
        local raw
        raw=$(ROCR_VISIBLE_DEVICES=$g0,$g1 mpirun -n 2 ${OSU_PT2PT}/osu_latency \
...
run_self_pair() {
    ...
    for run in $(seq 1 $NUM_RUNS); do
        if (( run == WARMUP_RUN )); then
            echo "  [run $run] warm-up — output discarded" | tee -a "$LOG_FILE"
            ROCR_VISIBLE_DEVICES=$g mpirun -n 2 ${OSU_PT2PT}/osu_latency -d rocm D D \
                > /dev/null 2>&1
            continue
        fi
        ...
        echo "  [run $run] recording" | tee -a "$LOG_FILE"
        local raw
        raw=$(ROCR_VISIBLE_DEVICES=$g mpirun -n 2 ${OSU_PT2PT}/osu_latency \
              -d rocm D D 2>/dev/null)
