#!/bin/bash
#SBATCH --job-name=osu_collectives_ucc_shm
...
#
# osu_collectives_ucc_shm.sh — A4 collectives with UCC shared-memory transport.
#
# Companion to osu_collectives.sh. Only difference: EESSI arm adds 'shm' to the
# UCC transport layer list (UCC_TLS=ucp,shm,self instead of ucp,self). Aims to
# attack the 1B-128B latency gap observed in osu_collectives_eessi vs native:
#   bcast       :  2.0 us (EESSI) vs 0.5 us (native)  — 4× gap
#   allgather   : 16.4 us         vs 3.7 us           — 4× gap
#   allreduce   :  4.7 us         vs 2.6 us           — 1.8× gap
#
# Hypothesis: with ucp,self only, UCC routes everything through point-to-point
# UCX. Adding 'shm' lets UCC use UCX's shared-memory transport directly inside
# the node, bypassing one layer of indirection. Single node, 8 ranks all on
# the same host — shm should be available and dominant.
#
# This script ISOLATES the SHM transport change. UCX_RNDV_THRESH stays at
# UCX default (so the 256B cliff will still be visible) — that's the variable
# osu_collectives_fixed_rndv.sh covers separately.
#
# See fix_collectives_reasoning.md for the full strategy.
#
# Caveat: if the UCC build in EESSI 2025.06 doesn't ship the shm TL, UCC will
# silently fall back to ucp; results will look identical to the baseline.
# Check the .log for UCC startup messages if so.
#
# Submit:
#   sbatch osu_collectives_ucc_shm.sh eessi
#   sbatch osu_collectives_ucc_shm.sh native     # baseline reference
...
FILE_BASE="osu_collectives_ucc_shm_${STACK}"
...
run_eessi() {
    local sdma=$1 benchmark=$2 bin=$3 ng=$4 devices=$5
    { echo; echo "# [eessi sdma=$sdma UCC_TLS=$UCC_TLS] $benchmark  N=$ng  devices=$devices"; } | tee -a "$LOG_FILE"
    for run in $(seq 1 $NUM_RUNS); do
        local t0=$SECONDS
        if (( run == WARMUP_RUN )); then
            echo "  [run $run/$NUM_RUNS] warm-up — $(date '+%H:%M:%S')" | tee -a "$LOG_FILE"
            timeout $RUN_TIMEOUT bash -c \
                "ROCR_VISIBLE_DEVICES=$devices mpirun -n $ng -x UCC_TLS -x UCC_CL_BASIC_TLS $bin -d rocm" \
                > /dev/null 2>&1
            echo "  [run $run/$NUM_RUNS] warm-up done in $((SECONDS - t0))s" | tee -a "$LOG_FILE"
            continue
        fi
...
if [[ "$STACK" == "native" ]]; then
    ...
    echo "######### EESSI STACK (HSA_ENABLE_SDMA=0, UCC_TLS=ucp,shm,self) ########" | tee -a "$LOG_FILE"
    ...
    # The change: add 'shm' to the UCC TL list. Lets UCC use UCX's
    # shared-memory transport directly inside the node.
    export UCC_TLS=ucp,shm,self
    export UCC_CL_BASIC_TLS=ucp,shm,self
