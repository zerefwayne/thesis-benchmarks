...
#
# osu_bw_host.sh — A3 host-buffer baseline (4 pairs)
# Reproduces paper "Trivial Staging" curve (arXiv:2408.14090v2 Sec III-C).
# One stack per job. Submit:
#   sbatch osu_bw_host.sh eessi
...
# H H = host buffers on both sides. No -d rocm.
OSU_FLAGS="-m 1:67108864 -i 100 H H"
...
run_pair_native() {
    ...
    { echo; echo "# [native sdma=$sdma host_buf] $label  GCD($g0,$g1) tier=$tier"; } | tee -a "$LOG_FILE"
...
run_pair_eessi() {
    ...
    local topo tier nlinks
    topo=$(get_topology "$g0" "$g1"); tier="${topo%,*}"; nlinks="${topo#*,}"
    { echo; echo "# [eessi sdma=$sdma host_buf] $label  GCD($g0,$g1) tier=$tier"; } | tee -a "$LOG_FILE"
    for run in $(seq 1 $NUM_RUNS); do
        local t0=$SECONDS
        if (( run == WARMUP_RUN )); then
            echo "  [run $run/$NUM_RUNS] warm-up — $(date '+%H:%M:%S')" | tee -a "$LOG_FILE"
