...
#
# osu_get_bw.sh — A8b inter-node one-sided RMA get bandwidth.
# One-sided OSU usage takes [options] only — no SRC/DST positionals; -d rocm
# puts the window on device memory.
...
OSU_FLAGS="-m 1:1048576 -i 100 -d rocm"
...
run_pair() {
    ...
    for run in $(seq 1 $NUM_RUNS); do
        ...
        if (( run != WARMUP_RUN )); then
            echo "$raw" | parse_rma "$STACK" "$sdma" "$lab" "$NODE_A" "$NODE_B" \
