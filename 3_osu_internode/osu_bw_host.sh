...
#
# osu_bw_host.sh — A3 inter-node host-buffer (H H) baseline.
# Same flags as osu_bw.sh except `-d rocm H H` — measures what the network
# does WITHOUT the GPU-Direct RDMA fast path. Reduced sweep (A3_HOST_PAIRS):
# one representative pair per inter-node NIC tier.
...
OSU_FLAGS="-m 1:1048576 -i 100 -d rocm H H"
...
run_pair() {
    local sdma=$1 g0=$2 g1=$3 lab=$4 nic=$5 bin=$6
    {
        echo
        echo "################################################################"
        echo "# [$STACK sdma=$sdma] $lab  GCD($g0,$g1)  nic=$nic  H H"
        echo "################################################################"
    } | tee -a "$LOG_FILE"
